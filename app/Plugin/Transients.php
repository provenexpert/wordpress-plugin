...
class Transients {
	...
	public function init(): void {
		...
		// use our own hooks.
		add_filter( 'provenexpert_transient_hide_on', array( $this, 'set_default_pages_where_transients_are_hidden' ) );

		// process AJAX-requests to dismiss transient notices.
		add_action( 'wp_ajax_provenexpert_dismiss_admin_notice', array( $this, 'dismiss_transient_via_ajax' ) );
	}
	...
	public function add_transient( Transient $transient_obj ): void {
		...
		update_option( PROVENEXPERT_TRANSIENTS_LIST, $transients_in_db );
	}
	...
	public function check_transients(): void {
		 ...
		 */
		foreach ( apply_filters( 'provenexpert_get_transients_for_display', $transients ) as $transient_obj ) {
	...
	public function dismiss_transient_via_ajax(): void {
		...
		// save value.
		delete_option( 'provenExpertDismissed-' . md5( $option_name ) );
		add_option( 'provenExpertDismissed-' . md5( $option_name ), $dismissible_length, '', true );
